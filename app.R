            ...
            column(
                ...
                shiny::wellPanel(
                    ...
                    pickerInput(inputId = "stock_selection", 
                                ...
                                choices = c("AAPL", "NVDA", "TSLA", "SQ", "NIO", "SPY", "BND"), 
                                multiple = FALSE,
                                selected = "TSLA", 
...
server <- function(input, output) {
    ...
    ## Event Reactive for Stock Selection
    stock_selection_reactive <- eventReactive(eventExpr = input$analyze, {
        input$stock_selection
    }, ignoreNULL = FALSE)
    
    ## Event Reactive - Date From
    date1_reactive <- eventReactive(eventExpr = input$analyze, {
        input$date1
    }, ignoreNULL = FALSE)
    
    ## Event Reactive - Date To
    date2_reactive <- eventReactive(eventExpr = input$analyze, {
        input$date2
    }, ignoreNULL = FALSE)
    
    ## Event Reactive - Monte-Carlo Trials
    mc_trials <- eventReactive(eventExpr = input$analyze, {
        input$trials
    }, ignoreNULL = FALSE)
